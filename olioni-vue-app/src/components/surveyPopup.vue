      ...
      <div class="responseWrap" v-for="(question, index) in qResponses" :key="index">
        <div class="responseContainer" v-if="index === qIndex" :id="index">
          <div class="response" :key="index" v-for="(items, index) in question" :id="index">
            <label class="switch">
              <input type="checkbox" @click="highlightRespo($event, index, question)">
              <span class="slider"> 
...
export default {
  ...
  methods: {
    ...
    highlightRespo(event, id, question) {
      ...
      if (event.target.checked == true) {
        this.selectedResponses.push(question[id])
        console.log('response added')
      } else {
        let removeIndex = this.selectedResponses.indexOf(question[id])
        this.selectedResponses.splice(removeIndex, 1)
      }
